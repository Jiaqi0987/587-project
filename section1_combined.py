...
class TransformerModel(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, lr, device_type):
        ...
        optimizer = torch.optim.AdamW(optimizer_groups, lr=lr, betas=(0.9, 0.95), eps=1e-8, fused=use_fused)
        return optimizer
...
class CustomDataLoader:
    def __init__(self, B, T, process_rank, num_processes, split):
        ...
        #loading the shard filenames
        self.shards = self._load_shard_filenames(split)
        if master_process:
    ...
    def _load_shard_filenames(self, split):
        data_root = "edu_fineweb10B"
        shards = sorted(
            os.path.join(data_root, shard)
            for shard in os.listdir(data_root)
            if split in shard
        )
    ...
    def reset(self):
        #reset the loader state to the first shard, then prepare tokens for loading
        self.current_shard = 0
        self.tokens = self._load_tokens_from_shard(self.shards[self.current_shard])
        self.current_position = self._calculate_initial_position()
    
    def _calculate_initial_position(self):
        #calculate the starting position in tokens
        return self.B * self.T * self.process_rank
    
    def _load_tokens_from_shard(self, shard_path):
        return load_tokens(shard_path)
    
    def next_batch(self):

        B, T = self.B, self.T

        #extracting the buffer for the batch
        ...
        #handle end-of-shard logic
        if self._is_end_of_shard():
            self._advanced_to_next_shard()
        
        return x, y
    
    def _is_end_of_shard(self):
        return self.current_position + (self.B * self.T * self.num_processes + 1) > len(self.tokens)
    
    def _advanced_to_next_shard(self):
